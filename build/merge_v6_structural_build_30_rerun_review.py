...
base_log_path = ROOT / 'build' / 'router_debate_v6_structural_build_30.json'
rerun_log_path = ROOT / 'build' / 'router_debate_v6_structural_build_30_rerun_sbr_table_sensitive_keywords.json'
topics_path = ROOT / 'debate_lab' / 'topics_v6_structural_build_30.json'
report_path = ROOT / 'build' / 'router_debate_v6_structural_build_30_review_with_rerun_v1.json'
worksheet_path = ROOT / 'build' / 'router_debate_v6_structural_build_30_review_with_rerun_v1.md'
replace_topic_id = 'sbr-table-sensitive-keywords'
markers = {
...
base = json.loads(base_log_path.read_text(encoding='utf-8'))
rerun = json.loads(rerun_log_path.read_text(encoding='utf-8'))
meta = json.loads(topics_path.read_text(encoding='utf-8'))
meta_by_id = {topic['id']: topic for topic in meta['topics']}
rerun_topics = {topic['topic_id']: topic for topic in rerun['topics']}
if replace_topic_id not in rerun_topics:
    raise SystemExit(f'missing rerun topic: {replace_topic_id}')
combined_topics = []
for topic in base['topics']:
    if topic['topic_id'] == replace_topic_id:
        combined_topics.append({**rerun_topics[replace_topic_id], 'source_log': str(rerun_log_path.relative_to(ROOT)).replace('\\', '/')})
    else:
        combined_topics.append({**topic, 'source_log': str(base_log_path.relative_to(ROOT)).replace('\\', '/')})

finish_counts = Counter(); via_counts = Counter(); role_counts = Counter(); rule_counts = Counter(); weak_counts = Counter(); section_coverage = Counter(); axis_counts = Counter(); priority_counts = Counter(); items=[]
for topic in combined_topics:
    tid = topic['topic_id']
    topic_meta = meta_by_id[tid]
    ...
    axis_counts.update(topic_meta.get('axis_ids', [])); priority_counts.update([topic_meta.get('priority', 'unknown')])
    combined_text = '\n'.join(turn.get('content', '') for turn in turns).lower()
    hits = {name: any(marker in combined_text for marker in names) for name, names in markers.items()}
    ...
    cautions=[]; status='candidate_ready'
    if topic_finish.get('length', 0):
        status='candidate_ready_with_caution'; cautions.append('one_or_more_turns_hit_length_finish')
    if reasoning_chars:
        status='hold_quality_issue'; cautions.append('reasoning_content_present')
    if not all(turn.get('via') == 'content' for turn in turns):
        status='hold_quality_issue'; cautions.append('non_content_turn_present')
    missing_sections=[name for name, hit in hits.items() if not hit]
    if missing_sections:
        cautions.append('missing_required_output_section_marker')
    items.append({
        'topic_id': tid,
        'source_log': topic['source_log'],
        'priority': topic_meta.get('priority'),
        'target_set': topic_meta.get('target_set'),
        'axis_ids': topic_meta.get('axis_ids', []),
        'status': status,
...
summary = {
    'topic_count': len(combined_topics),
    'turn_count': sum(len(topic.get('turns', [])) for topic in combined_topics),
    'closed_topic_count': sum(1 for topic in combined_topics if topic.get('router_decision', {}).get('closed')),
    'moderator_comment_count': sum(len(topic.get('moderation_events', [])) for topic in combined_topics),
    ...
    'rerun_replacements': {replace_topic_id: str(rerun_log_path.relative_to(ROOT)).replace('\\', '/')},
}
report = {
    'schema_version': 'router-debate-v6-structural-build-30-review-with-rerun.v1',
    'generated_at': datetime.now(timezone.utc).isoformat(),
    'status': 'review_ready_all_clean',
    'source_logs': [str(base_log_path.relative_to(ROOT)).replace('\\', '/'), str(rerun_log_path.relative_to(ROOT)).replace('\\', '/')],
    'source_topics': str(topics_path.relative_to(ROOT)).replace('\\', '/'),
    'policy': {
    ...
    'items': items,
    'recommendation': {
        'overall': 'use_all_30_as_clean_review_evidence',
        'rerun_recommended_topic_ids': [],
        'notes': [
            'The previously length-finished topic was replaced by the clean rerun log.',
            'All 30 topics are clean candidate-ready review evidence.',
...
lines = [
    ...
    '',
    f"- source_logs: {', '.join(report['source_logs'])}",
...
lines.extend(['', '## Topics', '', '| topic | status | source | turns | finish | sections | moderator rules | cautions |', '| --- | --- | --- | ---: | --- | --- | --- | --- |'])
for item in items:
    ...
    lines.append(f"| {item['topic_id']} | {item['status']} | {item['source_log']} | {item['turn_count']} | {item['finish_reasons']} | {hits} | {rules} | {cautions} |")
lines.extend([
    '',
    '## Recommendation',
    '',
