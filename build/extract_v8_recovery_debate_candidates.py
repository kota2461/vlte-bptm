"""Extract reviewable V8 recovery candidates from a round-4 debate run.
...
DEFAULT_TOPICS_PATH = ROOT / "debate_lab" / "topics_v8_recovery_100.json"
DEFAULT_SELECTION_PATH = ROOT / "build" / "v8_recovery_debate_candidate_selection_v1.json"
DEFAULT_WORKSHEET_PATH = ROOT / "build" / "v8_recovery_debate_candidate_review_worksheet_v1.md"
...
def _score_result(result: dict[str, Any], topic_meta: dict[str, Any], *, min_rounds: int) -> tuple[int, list[str], list[str]]:
    ...
    if completed_rounds >= min_rounds:
        score += 25
        reasons.append("round4_complete")
    else:
        cautions.append("below_round4")
    ...
    if {"missing_info", "current_search_split", "unverified_claim", "constraints", "operation_terminal", "risk_ladder"} & axes:
        score += 8
        reasons.append("direct_v7_miss_surface")
...
def _candidate_status(score: int, cautions: list[str], min_score: int) -> str:
    if score >= min_score and "below_round4" not in cautions and "suppressed_or_noncontent_turn" not in cautions:
...
def build_selection(source_log: Path, topics_path: Path, *, min_score: int, min_rounds: int, top: int | None) -> dict[str, Any]:
    ...
    for index, result in enumerate(run.get("topics", []), start=1):
        ...
        candidates.append(
            {
                "id": f"v8-recovery-debate-candidate-{index:03d}",
                "status": status,
    ...
    return {
        ...
        "status": "candidate_queue_prepared_from_round4_debate_log",
...
def write_worksheet(path: Path, selection: dict[str, Any]) -> None:
    ...
    for candidate in selection["candidates"]:
        ...
        lines.append(
            ...
            f"{candidate['source_topic_id']} | {candidate['recovery_focus']} | {candidate['completed_rounds']} | "
            f"{candidate['content_chars']} | {cautions} | {theme} |"
        )
    lines.extend(
        [
            "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract V8 recovery debate candidates.")
