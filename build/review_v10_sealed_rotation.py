"""Review readiness to rotate a fresh PLM sealed v10 fixture.

This Step 4 review is deliberately pre-rotation: it does not create, open, or
measure a sealed v10 fixture. It verifies that the V10 mainline non-sealed
replay gate is clean, sealed v9 is consumed, and no active sealed fixture
exists.
"""
...
OUTPUT_JSON = ROOT / "build" / "v10_sealed_rotation_review_v1.json"
OUTPUT_MD = ROOT / "build" / "v10_sealed_rotation_review_v1.md"
...
def build_report() -> dict[str, Any]:
    ...
    v9_measurement = _load_json(V9_MEASUREMENT_PATH)
    active = _active_sealed(registry)
    v9_registry = registry["fixtures"].get("pattern_language_sealed_v9.json", {})

    blockers: list[str] = []
    if gate["schema_version"] != "v10-mainline-replay-gate-report.v1":
        blockers.append("unexpected_step3_gate_schema")
    if gate["status"] != "passed" or gate["passed"] is not True:
        blockers.append("v10_mainline_replay_gate_not_passed")
    if gate["summary"]["ready_for_step4_sealed_v10_rotation_review"] is not True:
    ...
    if gate["contract"]["can_use_as_training_data"] is not False:
        blockers.append("mainline_gate_contract_allows_training_data")
    ...
    if v9_registry.get("status") != "consumed" or v9_registry.get("measured") is not True:
        blockers.append("previous_sealed_v9_not_consumed")
    if v9_measurement["registry_update"]["rotation_required_before_tuning"] is not True:
        blockers.append("v9_measurement_does_not_require_rotation")
    if v9_measurement["sealed_labels_used_for_tuning"] is not False:
        blockers.append("sealed_v9_labels_used_for_tuning")
    if active:
        blockers.append("active_sealed_fixture_already_exists")
    if SEALED_V10_PATH.exists():
        blockers.append("sealed_v10_fixture_already_exists")

    decision = "eligible_for_fresh_sealed_v10_rotation" if not blockers else "blocked"
    return {
        "schema_version": "v10-sealed-rotation-review.v1",
        ...
        "sources": {
            ...
            "previous_measurement": _rel(V9_MEASUREMENT_PATH),
        },
        ...
        "registry_state": {
            ...
            "previous_sealed": {
                ...
                "status": v9_registry.get("status"),
                "measured": v9_registry.get("measured"),
                "reviewed": v9_registry.get("reviewed"),
                "measurement_report": v9_registry.get("measurement_report"),
            },
            "planned_successor": {
        ...
        "previous_measurement_summary": {
            "fixture": v9_measurement["fixture"]["registry_name"],
            "sealed_fixture_opened": v9_measurement["sealed_fixture_opened"],
            "sealed_labels_used_for_tuning": v9_measurement["sealed_labels_used_for_tuning"],
            "intent_accuracy": v9_measurement["measurements"]["intent_accuracy"],
            "critical_signal_recall": v9_measurement["measurements"]["critical_signal_recall"],
            "operation_exact_match": v9_measurement["measurements"]["operation_exact_match"],
            "constraint_exact_match": v9_measurement["measurements"]["constraint_exact_match"],
            "risk_exact_match": v9_measurement["measurements"]["risk_exact_match"],
            "error_count": len(v9_measurement["measurements"]["errors"]),
            "rotation_required_before_tuning": v9_measurement["registry_update"]["rotation_required_before_tuning"],
        },
        "planned_v10_fixture_constraints": {
            "case_count": 28,
            "cases_per_intent": 4,
            "split": "sealed",
            ...
            "must_exclude_exact_text_from": {
                ...
                "v10_required_nonsealed_gate_lanes": [lane["source"] for lane in gate["required_lanes"]],
            },
        ...
        "blockers": blockers,
        "next_action": (
            "roadmap_v10_step5_generate_and_rotate_sealed_v10_fixture"
            if not blockers
            else "resolve_blockers_before_sealed_v10_rotation"
        ),
    }
...
def _write_markdown(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# V10 Sealed Rotation Review v1",
        "",
        f"- status: `{report['decision']}`",
        f"- passed: `{report['passed']}`",
        f"- sealed_v10_fixture_created_now: `{report['policy']['sealed_v10_fixture_created_now']}`",
        ...
        "",
        (
            "Fresh sealed v10 rotation is eligible. This review does not create "
            "or open the sealed v10 fixture; it only authorizes the next roadmap "
            "step to generate and rotate it."
            if report["passed"]
            else "Fresh sealed v10 rotation is blocked. Resolve blockers first."
        ),
        "",
