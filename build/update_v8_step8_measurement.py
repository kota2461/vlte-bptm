"""Record V8 Step 8 sealed measurement state in targets and roadmaps."""
...
def build_update() -> dict[str, Any]:
    ...
    targets["status"] = "step8_sealed_v8_measurement_completed_v9_rotation_required"
    targets["next_action"] = "roadmap_v9_step1_post_v8_measurement_taxonomy"
    ...
    lines.extend(
        [
            ...
            "",
            (
                "V8 met the sealed minimum target, but the sealed v8 fixture is consumed. "
                "Sealed labels remain measurement-only, and fresh rotation is still required before any tuning from this result."
                if passed_minimum
                else "V8 did not meet the sealed minimum target. The sealed v8 fixture is consumed, sealed labels remain measurement-only, and fresh rotation is required before tuning from this result."
            ),
        ]
    )
    ...
    section = (
        "## Step 8 Output\n\n"
        "`build\\pattern_language_sealed_v8_measurement_report.json` measured the active sealed v8 fixture once and consumed it. "
        ...
        "Sealed labels remain measurement-only and V9 taxonomy/rotation is required before tuning."
    )
