"""Adopt V6 structural build candidates for non-sealed replay.

The user confirmed the 30 structural build-vs-respond candidates should be
adopted. This records the decision and creates a human-reviewed benchmark lane.
It is still non-sealed, not gate evidence, and not direct training data.
"""

import json
import sys
from collections import Counter
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from semantic_routing import evaluate_plm_extractor, parse_plm_benchmark, route  # noqa: E402

SOURCE_QUEUE_PATH = ROOT / "build" / "v6_structural_build_30_candidate_queue_v1.json"
SOURCE_REPORT_PATH = ROOT / "build" / "v6_structural_build_30_candidate_probe_report_v1.json"
SOURCE_BENCHMARK_PATH = ROOT / "tests" / "fixtures" / "v6_structural_build_30_candidate_benchmark_v1.json"
ADOPTED_BENCHMARK_PATH = ROOT / "tests" / "fixtures" / "v6_structural_build_30_adopted_benchmark_v1.json"
ADOPTION_DECISION_PATH = ROOT / "build" / "v6_structural_build_30_adoption_decision_v1.json"
REPLAY_REPORT_PATH = ROOT / "build" / "v6_structural_build_30_adopted_replay_report_v1.json"
WORKSHEET_PATH = ROOT / "build" / "v6_structural_build_30_adopted_worksheet_v1.md"

POLICY = {
    "sealed_fixtures_used_as_sources": False,
    "sealed_text_used": False,
    "sealed_labels_used": False,
    "raw_debate_logs_direct_training_allowed": False,
    "llm_turn_text_direct_training_allowed": False,
    "topic_metadata_synthesis_used": True,
    "manual_prompt_synthesis_used": True,
    "human_review_confirmation_recorded": True,
    "same_cycle_promotion_allowed": False,
    "current_route_measurement_is_gate": False,
    "adopted_benchmark_is_directly_trainable": False,
}


def load_json(path: Path) -> Any:
    return json.loads(path.read_text(encoding="utf-8"))


def write_json(path: Path, payload: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
        newline="\n",
    )


def rel(path: Path) -> str:
    return path.relative_to(ROOT).as_posix()


def compact_measurement(measurement: dict[str, Any]) -> dict[str, Any]:
    error_fields = Counter(
        field for error in measurement["errors"] for field in error.get("fields", [])
    )
    return {
        "case_count": measurement["case_count"],
        "intent_accuracy": measurement["intent_accuracy"],
        "critical_signal_recall": measurement["critical_signal_recall"],
        "operation_exact_match": measurement["operation_exact_match"],
        "constraint_exact_match": measurement["constraint_exact_match"],
        "risk_exact_match": measurement["risk_exact_match"],
        "valid_packet_rate": measurement["valid_packet_rate"],
        "error_count": len(measurement["errors"]),
        "error_field_counts": dict(sorted(error_fields.items())),
    }


def assert_ready_for_adoption(
    source_benchmark: dict[str, Any],
    source_report: dict[str, Any],
    source_queue: dict[str, Any],
) -> None:
    if source_benchmark["schema_version"] != "pattern-language-benchmark.v1":
        raise ValueError("source benchmark must be a PLM benchmark")
    if source_benchmark["review_status"] != "draft":
        raise ValueError("source benchmark should remain draft before adoption")
    if source_report["schema_version"] != "v6-structural-build-30-candidate-probe-report.v1":
        raise ValueError("unsupported source report schema")
    if source_report["status"] != "candidate_extraction_complete_review_required":
        raise ValueError("source report is not ready for adoption review")
    if source_queue["schema_version"] != "v6-structural-build-30-candidate-queue.v1":
        raise ValueError("unsupported source queue schema")
    if source_queue["status"] != "candidate_queue_ready_for_human_review":
        raise ValueError("source queue is not ready for human review")
    if source_report["policy"] != source_queue["policy"]:
        raise ValueError("source queue/report policy mismatch")
    if source_queue["policy"]["raw_debate_logs_direct_training_allowed"] is not False:
        raise ValueError("raw debate logs must remain excluded from training")
    if source_queue["policy"]["human_review_required_before_training"] is not True:
        raise ValueError("source queue must require review before training")
    if len(source_benchmark["cases"]) != 30 or len(source_queue["candidates"]) != 30:
        raise ValueError("expected exactly 30 structural build candidates")
    if any(case["expected"]["primary_intent"] != "build" for case in source_benchmark["cases"]):
        raise ValueError("all structural build adopted cases must expect build")
    if any(case["expected"]["risk"]["level"] != "low" for case in source_benchmark["cases"]):
        raise ValueError("all structural build adopted cases must expect low risk")
    if any(candidate["training_allowed"] for candidate in source_queue["candidates"]):
        raise ValueError("candidate queue must not mark candidates as training allowed")
    if any(candidate["gate_use_allowed"] for candidate in source_queue["candidates"]):
        raise ValueError("candidate queue must not mark candidates as gate-use allowed")


def summarize_queue(source_queue: dict[str, Any], measurement: dict[str, Any]) -> dict[str, Any]:
    candidates = source_queue["candidates"]
    by_priority = Counter(candidate["priority"] for candidate in candidates)
    by_axis: Counter[str] = Counter()
    for candidate in candidates:
        by_axis.update(candidate["axis_ids"])
    compact = compact_measurement(measurement)
    return {
        "adopted_count": len(candidates),
        "by_priority": dict(sorted(by_priority.items())),
        "by_expected_intent": {"build": len(candidates)},
        "by_expected_risk": {"low": len(candidates)},
        "axis_counts_top": dict(by_axis.most_common(16)),
        "current_route_measurement": compact,
    }


def write_worksheet(source_queue: dict[str, Any], report: dict[str, Any]) -> None:
    candidates = source_queue["candidates"]
    measurement = report["measurement"]
    lines = [
        "# V6 Structural Build 30 Adopted Worksheet v1",
        "",
        "Human-reviewed non-sealed replay lane for structural build-vs-respond repair.",
        "This is not sealed, not gate evidence, and not direct training data.",
        "",
        "## Summary",
        "",
        f"- adopted_count: {report['summary']['adopted_count']}",
        f"- current_route_error_count: {measurement['error_count']}",
        f"- current_route_intent_accuracy: {measurement['intent_accuracy']:.3f}",
        f"- current_route_operation_exact_match: {measurement['operation_exact_match']:.3f}",
        f"- current_route_risk_exact_match: {measurement['risk_exact_match']:.3f}",
        "- training_allowed: false",
        "- gate_use_allowed: false",
        "",
        "## Adopted Candidates",
        "",
        "| adopted_id | candidate_id | score | topic | priority | current | gaps | overfire | prompt |",
        "| --- | --- | ---: | --- | --- | --- | --- | --- | --- |",
    ]
    for index, candidate in enumerate(candidates, start=1):
        current = candidate["current_route"]
        prompt = candidate["prompt"].replace("|", "&#124;").replace("\n", "<br>")
        lines.append(
            "| "
            f"v6-structural-build-adopted-{index:03d} | {candidate['id']} | {candidate['selection_score']} | "
            f"{candidate['source_topic_id']} | {candidate['priority']} | "
            f"{current['primary_intent']}:{current['risk']['level']} | "
            f"{','.join(candidate['current_route_gaps'])} | {','.join(candidate['overfire_flags'])} | {prompt} |"
        )
    lines.extend([
        "",
        "## Contract",
        "",
        "- current_route_measurement_is_gate: false",
        "- same_cycle_promotion_allowed: false",
        "- adopted_benchmark_is_directly_trainable: false",
    ])
    WORKSHEET_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")


def main() -> None:
    generated_at = datetime.now(timezone.utc).isoformat()
    source_benchmark = load_json(SOURCE_BENCHMARK_PATH)
    source_report = load_json(SOURCE_REPORT_PATH)
    source_queue = load_json(SOURCE_QUEUE_PATH)
    parse_plm_benchmark(source_benchmark)
    assert_ready_for_adoption(source_benchmark, source_report, source_queue)

    adopted_benchmark = {
        "schema_version": "pattern-language-benchmark.v1",
        "frozen_at": generated_at,
        "authoring_method": (
            "V6 structural build-vs-respond candidates adopted after user confirmation; "
            "short prompts synthesized from non-sealed router debate topic metadata"
        ),
        "review_status": "human_reviewed",
        "policy": (
            "Human-reviewed non-sealed structural-build replay lane. Raw debate turns are not direct training data. "
            "This is not a sealed fixture, not a promotion gate, and not same-cycle promotion evidence."
        ),
        "cases": source_benchmark["cases"],
    }
    adopted = parse_plm_benchmark(adopted_benchmark)
    measurement = evaluate_plm_extractor(adopted.cases, lambda text: route(text).packet)
    compact = compact_measurement(measurement)
    summary = summarize_queue(source_queue, measurement)

    decision = {
        "schema_version": "v6-structural-build-30-adoption-decision.v1",
        "generated_at": generated_at,
        "status": "adopted_for_nonsealed_replay",
        "review_status": "human_reviewed",
        "reviewed_by": "user_confirmation_in_codex_thread",
        "source_candidate_queue": rel(SOURCE_QUEUE_PATH),
        "source_candidate_report": rel(SOURCE_REPORT_PATH),
        "source_candidate_benchmark": rel(SOURCE_BENCHMARK_PATH),
        "adopted_benchmark": rel(ADOPTED_BENCHMARK_PATH),
        "worksheet": rel(WORKSHEET_PATH),
        "adopted_count": len(adopted_benchmark["cases"]),
        "case_ids": [case["id"] for case in adopted_benchmark["cases"]],
        "policy": POLICY,
    }
    has_gaps = bool(compact["error_count"])
    report = {
        "schema_version": "v6-structural-build-30-adopted-replay-report.v1",
        "generated_at": generated_at,
        "status": "completed_with_expected_route_gaps" if has_gaps else "completed_without_route_gaps",
        "adoption_decision": rel(ADOPTION_DECISION_PATH),
        "adopted_benchmark": rel(ADOPTED_BENCHMARK_PATH),
        "worksheet": rel(WORKSHEET_PATH),
        "current_route_measurement_is_gate": False,
        "sealed_fixture_used": False,
        "policy": POLICY,
        "summary": summary,
        "measurement": compact,
        "errors": measurement["errors"],
        "interpretation": (
            "The adopted structural-build lane intentionally exposes current respond/verify/search overfire gaps. Use it for non-sealed replay improvements, not as a sealed gate."
            if has_gaps
            else "The adopted structural-build lane currently matches route(). It remains non-sealed replay evidence, not a sealed gate."
        ),
        "next_step": "Improve build-vs-respond structural routing, then replay this adopted non-sealed lane.",
    }

    write_json(ADOPTED_BENCHMARK_PATH, adopted_benchmark)
    write_json(ADOPTION_DECISION_PATH, decision)
    write_json(REPLAY_REPORT_PATH, report)
    write_worksheet(source_queue, report)
    print(json.dumps({
        "status": report["status"],
        "adopted_benchmark": rel(ADOPTED_BENCHMARK_PATH),
        "adopted_count": decision["adopted_count"],
        "measurement": compact,
    }, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()