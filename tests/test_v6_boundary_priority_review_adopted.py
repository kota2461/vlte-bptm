...
SCRIPT_PATH = ROOT / "build" / "adopt_v6_boundary_priority_review_candidates.py"
...
def test_v6_boundary_priority_review_adopted_benchmark_is_human_reviewed() -> None:
    ...
    assert len({case.input_text for case in benchmark.cases}) == 26
    assert "README" in benchmark.cases[14].input_text
    assert "mojibake" not in payload["policy"].lower()


def test_v6_boundary_priority_review_adoption_decision_records_policy() -> None:
    decision = _load(ADOPTION_DECISION_PATH)

    assert decision["schema_version"] == "v6-boundary-priority-review-adoption-decision.v1"
    ...
    assert decision["adopted_count"] == 26
    assert decision["selected_queue_ids"][0] == "v6-boundary-debate-queue-034"
    assert decision["selected_queue_ids"][-1] == "v6-boundary-debate-queue-048"
...
def test_v6_boundary_priority_review_replay_report_is_not_gate() -> None:
    report = _load(REPLAY_REPORT_PATH)

    assert report["schema_version"] == "v6-boundary-priority-review-adopted-replay-report.v1"
    ...
    assert report["summary"]["adopted_count"] == 26
    assert report["summary"]["by_action"] == {
        "coverage_keep": 22,
        "priority_contrast_negative_review": 1,
        "priority_suppression_review": 3,
    }
    assert report["summary"]["by_target_set"] == {
        "contrast_negative_repair": 16,
        "false_positive_set": 1,
        "mixed_ja_en": 3,
        "no_risk_contrast": 2,
        "paraphrase_set": 4,
    }
...
def test_v6_boundary_priority_review_lane_exposes_current_route_gaps() -> None:
    ...
    assert measurement["case_count"] == 26
...
def test_v6_boundary_priority_review_worksheet_exists() -> None:
    text = WORKSHEET_PATH.read_text(encoding="utf-8")

    assert "V6 Boundary Priority Review Adopted Worksheet v1" in text
    assert "adopted_count: 26" in text
    assert "current_route_error_count: 0" in text
    assert "v6-boundary-priority-review-001" in text
    assert "repair-license-label-use" in text
    assert "Tell me the PowerShell command for checking the current folder" in text


def test_v6_boundary_priority_review_adoption_script_regenerates_artifacts() -> None:
    ...
    report = _load(REPLAY_REPORT_PATH)
    assert report["measurement"]["case_count"] == 26
