...
def test_v9_sealed_measurement_consumes_fixture_once() -> None:
    ...
    assert measurement["fixture"]["registry_name"] == SEALED_V9_PATH.name
    ...
    assert entry["measurement_report"] == r"build\pattern_language_sealed_v9_measurement_report.json"


def test_v9_sealed_measurement_metrics_are_recorded_and_below_target() -> None:
    measurement = _load(MEASUREMENT_PATH)
    ...
    assert metrics["intent_accuracy"] == 0.892857
    assert metrics["intent_macro_f1"] == 0.895692
    assert metrics["critical_signal_recall"] == 0.857143
    assert metrics["operation_exact_match"] == 0.821429
    assert metrics["constraint_exact_match"] == 0.642857
    assert metrics["risk_exact_match"] == 0.75
    assert metrics["evidence_offset_validity"] == 1.0
    assert len(metrics["errors"]) == 17
...
def test_v9_measurement_blocks_readiness_until_successor_rotation() -> None:
    ...
    assert targets["step8_sealed_measurement"]["critical_signal_miss_gate_met"] is False
    ...
    assert "V9 Step 8 Sealed Measurement Summary" in summary
