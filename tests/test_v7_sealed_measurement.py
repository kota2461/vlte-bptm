...
def test_v7_sealed_measurement_consumes_fixture_once() -> None:
    ...
    assert measurement["fixture"]["registry_name"] == SEALED_V7_PATH.name
    ...
    assert entry["measurement_report"] == r"build\pattern_language_sealed_v7_measurement_report.json"


def test_v7_sealed_measurement_metrics_are_recorded_and_below_target() -> None:
    measurement = _load(MEASUREMENT_PATH)
    ...
    assert metrics["intent_macro_f1"] == 0.761946
    assert metrics["critical_signal_recall"] == 0.642857
    assert metrics["operation_exact_match"] == 0.714286
    assert metrics["constraint_exact_match"] == 0.75
    assert metrics["risk_exact_match"] == 0.785714
    assert metrics["evidence_offset_validity"] == 1.0
    assert len(metrics["errors"]) == 16
...
def test_v7_measurement_blocks_readiness_until_successor_rotation() -> None:
    ...
    assert "V7 Step 8 Sealed Measurement Summary" in summary
