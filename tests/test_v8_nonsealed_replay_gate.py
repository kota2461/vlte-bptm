...
SCRIPT_PATH = ROOT / "build" / "run_v8_nonsealed_replay_gate.py"
...
def test_v8_nonsealed_replay_gate_passes_contract() -> None:
    report = _load(REPORT_PATH)

    assert report["schema_version"] == "v8-nonsealed-replay-gate-report.v1"
    ...
    assert report["summary"] == {
        ...
        "dependency_v7_required_error_count": 0,
        "dependency_v7_diagnostic_error_count": 0,
        "required_lane_count": 1,
        "required_passed_lane_count": 1,
        "required_error_count": 0,
        "v8_priority_review_case_count": 30,
        "v8_priority_review_exact": True,
        "ready_for_step6_sealed_v8_rotation_review": True,
    }
    assert report["required_lanes"][0]["name"] == "v8_recovery_priority_review_approved"
    assert report["required_lanes"][0]["review_status"] == "human_reviewed"
    assert report["required_lanes"][0]["measurement"]["error_count"] == 0
    ...
    assert report["next_action"] == "roadmap_v8_step6_sealed_rotation_review"


def test_v8_nonsealed_replay_gate_matches_current_route() -> None:
    report = _load(REPORT_PATH)
...
def test_v8_nonsealed_replay_gate_markdown_and_script_regenerate() -> None:
    text = REPORT_MD_PATH.read_text(encoding="utf-8")

    assert "V8 Non-Sealed Replay Gate Report v1" in text
    assert "status: `passed`" in text
    assert "required_error_count: 0" in text
    assert "prior_provisional_replay_was_gate: false" in text
