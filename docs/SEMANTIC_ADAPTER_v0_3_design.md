...
> **NOTE (2026-06-16, read first).** The original v0.3 target in this document
> was `end_to_end ≥ 0.90`. After probe + data-collection rounds that target was
> **deferred to a future successor** (see §11 and
> `docs/INTENT_MODEL_v0_3_FROZEN_2026-06-16.md`). The **frozen v0.3 acceptance
> bar** is: beats v0.2 markers (intent 0.80→0.86, e2e 0.76→0.82) · keeps
> `critical_underprocessing = 0` · no regression on v0.2 strengths · deployable
> as the first baseline. v0.3 is **FROZEN and complete against this bar** — it
> is NOT "incomplete because 0.90 is unmet". 0.90 belongs to the v0.4+ successor.
